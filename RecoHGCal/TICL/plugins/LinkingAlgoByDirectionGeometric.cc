...
void LinkingAlgoByDirectionGeometric::findTrackstersInWindow(const std::vector<Trackster> &tracksters,
    ...
    for (int eta_i = search_box[0]; eta_i <= search_box[1]; ++eta_i) {
      for (int phi_i = search_box[2]; phi_i <= search_box[3]; ++phi_i) {
        ...
        for (const unsigned &t_i : in_tile) {
          if (t_i != seedId) {
            ...
            //calculate lenght of trackster
            if (t.vertices().size() >= 10 || tracksterInTile.vertices().size() >= 10) {
              auto min_z = 9999.;
              auto max_z = -1.;
              ...
              auto lenght_trackster = max_z - min_z;
              if (tracksterInTile_z < trackster_z + lenght_trackster / 2. &&
                  tracksterInTile_z > trackster_z - lenght_trackster / 2.) {
                if (sep2 < 1.) {
      ...
      for (const unsigned &index : indices) {
        ...
        if (!mask[t_i]) {
          ...
          resultCollectionSep[seedId].push_back(sqrt(distances2[index]));
          std::cout << "Index " << index << " sep " << sqrt(distances2[index]) << std::endl;
          if (useMask)
            mask[t_i] = 1;
        }
      }
  ...
  void DFSVisits(std::vector<std::vector<unsigned>> & graph,
                 std::vector<std::vector<float>> & graphSep,
    ...
    for (auto j = graph[u].begin(); j != graph[u].end(); j++) {
      if (!visits[*j]) {
        ...
        std::cout << tabs << "Visiting " << *j << " Energy " << thisTrackster.raw_energy() << " Position "
                  << thisTrackster.barycenter() << " Number of Vertices " << thisTrackster.vertices().size()
                  << " Distance " << graphSep[u][indexT] << std::endl;
        tabs.push_back('\t');
        indexT++;
        ...
        DFSVisits(graph, graphSep, visits, outTrackster, resultCollectionIndices, tracksters, layerClusters, *j, tabs);
        if (j == graph[u].end() - 1) {
          tabs.pop_back();
        }
      }
    ...
    for (int i = 0; i < graphSize; i++) {
      ...
      if (!visits[i]) {
        ...
        std::cout << " -- Trackster " << i << " Energy " << outTrackster.raw_energy() << " Position "
                  << outTrackster.barycenter() << " Number of Vertices " << outTrackster.vertices().size() << std::endl;
        DFSVisits(graph, graphSep, visits, outTrackster, resultCollectionIndices, tracksters, layerClusters, i, tabs);

        std::cout << " -- Out Trackster";
        for (auto const &idx : resultCollectionIndices[i]) {
          std::cout << " " << idx;
        }
        std::cout << " Energy " << outTrackster.raw_energy() << " Position " << outTrackster.barycenter()
                  << " Number of Vertices " << outTrackster.vertices().size() << std::endl;
  ...
  void LinkingAlgoByDirectionGeometric::linkTracksters(const edm::Handle<std::vector<reco::Track>> tkH,
    ...
    for (unsigned i = 0; i < tracks.size(); ++i) {
      ...
      if (LinkingAlgoBase::algo_verbosity_ > VerbosityLevel::Advanced)
        LogDebug("LinkingAlgoByDirectionGeometric") << "track " << i << " - eta " << tk.eta() << " phi " << tk.phi()
                                                    << " time " << tkTime[reco::TrackRef(tkH, i)] << " time qual "
                                                    << tkTimeQual[reco::TrackRef(tkH, i)] << "  muid " << muId << "\n";
    ...
    std::vector<std::vector<float>> trackstersResultsSep(tracksters.size());
    findTrackstersInWindow(tracksters,
                           layerClusters,
                           tracksterPropTiles,
                           del_tk_ts_layer1_,
                           separation_threshold_,
                           trackstersResults,
                           trackstersResultsSep,
                           distancesVec,
                           distancesVecIdx);
