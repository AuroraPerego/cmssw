...
void MtdTruthAccumulator::finalizeEvent(edm::Event &event, edm::EventSetup const &setup) {
  ...
  for (const auto &sc : *(output_.pSimClusters)) {
    ...
    for (unsigned int i = 0; i < sc.hits_and_fractions().size(); ++i) {
      ...
      IfLogDebug(DEBUG, messageCategory_)
          << std::fixed << std::setprecision(3) << " hit " << id.rawId() << " on layer " << geomTools_.getLayer(id)
          << " module " << geomTools_.getModule(id) << " row " << (unsigned int)(sc.detIds_and_rows()[i].second).first
  ...
  for (const auto &sc : *scHandle) {
    ...
    int prev = indices[0];
    DetId prevId(hAndF[prev].first >> 16);
    ...
    auto update_clu_info = [&](const int &ind) {
      ...
      auto position =
          geomTools_.getPosition((DetId)hAndR[ind].first, (hAndR[ind].second).first, (hAndR[ind].second).second).first;
      SimLCenergy += energy;
    ...
    for (const auto &ind : indices) {
      if (ind == indices[0])
        continue;
      DetId id(hAndF[ind].first >> 16);
      if (geomTools_.isETL(id) != geomTools_.isETL(prevId) or geomTools_.getLayer(id) != geomTools_.getLayer(prevId) or
    ...
    GlobalPoint posAtEntrance = geomTools_
                                    .getPosition((DetId)hAndR[idAtEntrance].first,
...
void MtdTruthAccumulator::fillSimHits(std::vector<std::pair<uint64_t, const PSimHit *>> &returnValue,
  ...
  for (auto const &collectionTag : collectionTags_) {
    ...
    for (auto const &simHit : *hSimHits) {
      ...
      std::pair<uint8_t, uint8_t> pixel = geomTools_.getPixelInModule(id, simscaled);
      // create the unique id
      uint64_t uniqueId = (uint64_t)id.rawId() << 16;
      uniqueId |= pixel.first << 8;
      ...
      IfLogDebug(DEBUG, messageCategory_)
          ...
          << geomTools_.getGlobalPosition(id, simscaled) << ", time(ns) " << simHit.tof() << ", energy(MeV) "
