...
void TICLCandidateValidator::fillCandidateHistos(const edm::Event& event,
  ...
  for (const auto i : chargedCandidates) {
    ...
    auto const sts_vecPU = mergeTsRecoToSimPUMap[mergeTs_id];
    if (!sts_vecPU.empty()) {
      const auto& stsPUAssociated = sts_vecPU[0];
      if (stsPUAssociated.second.first / (*Tracksters_h)[mergeTs_id].raw_energy() > 0.95)
        continue;
    }
